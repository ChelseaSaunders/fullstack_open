...
const calculateExercises = (dailyExerciseHours: Array<number>, targetHours: number) : exerciseOutput | errorOutput => {
  if (!dailyExerciseHours || !targetHours) {
    return {
      error: "parameters missing"
    }
  }

  if (dailyExerciseHours.filter(n => isNaN(n)).length > 0 || isNaN(targetHours)) {
    return {
